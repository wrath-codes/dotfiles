...
function get_active_workspaces()
    local file = io.popen("aerospace list-windows --all --format '%{workspace}'")
    local result = file:read("*a")
    file:close()

    local workspaces_set = {}
    for ws in result:gmatch("[^\n]+") do
        workspaces_set[ws] = true
    end

    local workspaces = {}
    for ws, _ in pairs(workspaces_set) do
        table.insert(workspaces, ws)
    end

    -- Sort numerically if workspace names are numbers
