...
function M.config()
	...
	for _, sign in
		ipairs(
			vim.tbl_get(
				vim.diagnostic.get(vim.api.nvim_get_current_buf(), { severity = vim.diagnostic.severity.ERROR }),
				"signs",
				"values"
			) or {}
		)
	do
		...
		vim.diagnostic.set_sign(vim.api.nvim_get_current_buf(), sign.name, sign.text, sign.numhl)
	end
