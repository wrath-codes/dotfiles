				...
				if opts.dap and Util.has("nvim-dap") and mason_registry.is_installed("java-debug-adapter") then
					...
					if opts.test and mason_registry.is_installed("java-test") then
						...
						if not Util.has("neotest") then
							wk.register({
                ["<leader>tj"] = { name = "+test (Java)" },
								["<leader>tja"] = { require("jdtls.dap").test_class, "Run All Test" },
								["<leader>tjn"] = { require("jdtls.dap").test_nearest_method, "Run Nearest Test" },
								["<leader>tjc"] = { require("jdtls.dap").pick_test, "Run Test" },
							}, { mode = "n", buffer = args.buf })
						end
					end
				end
