    ...
    require("snacks").picker({
      ...
      confirm = function(picker, item)
        ...
        require("snacks").picker({
          ...
          actions = {
            delete_prompt = function(picker2, item2)
              if prompts_utils.delete_prompt(item2.id) then
                vim.notify("Prompt '" .. item2.title .. "' deleted", vim.log.levels.INFO)
                -- Refresh the list
                vim.schedule(function()
                  vim.cmd("AmpDashXCategories")
                end)
            ...
            edit_prompt = function(picker2, item2)
              -- Construct full prompt data object for manage.edit_prompt
              local prompt_data = {
                id = item2.id,
                title = item2.title,
              ...
              require("utils.amp.commands.prompts.manage").edit_prompt(prompt_data)
            end,
            change_category = function(picker2, item2)
              picker2:close()
              require("utils.amp.commands.prompts.list").change_category(item2)
            end,
            send_prompt = function(picker2, item2)
              picker2:close()
              local cmd = "amp -x " .. vim.fn.shellescape(item2.prompt) .. "; echo '\nPress any key to close...'; read -n 1"
            ...
            copy_command = function(picker2, item2)
              local command = 'amp -x "' .. item2.prompt:gsub('"', '\\"') .. '"'
          ...
          win = {
            input = {
              keys = {
                ...
                ["<C-S-s>"] = { "change_category", mode = { "i", "n" } },
          ...
          on_show = function(picker2)
            if not picker2.hints_win or not vim.api.nvim_win_is_valid(picker2.hints_win) then
              local buf = vim.api.nvim_create_buf(false, true)
              local col_start = math.floor(0.1 * vim.o.columns)
              local width = math.floor(0.8 * vim.o.columns)
              local row = math.floor(0.9 * vim.o.lines)
              ---@diagnostic disable: inject-field
              picker2.hints_win = vim.api.nvim_open_win(buf, false, {
                relative = "editor",
                width = width,
                height = 4,
                row = row,
              ...
              -- Line 0: delete + edit
              vim.hl.range(buf, ns, "SnacksPickerLabel", { 0, 0 }, { 0, 9 })
              vim.hl.range(buf, ns, "Comment", { 0, 9 }, { 0, 55 })
              vim.hl.range(buf, ns, "SnacksPickerLabel", { 0, 55 }, { 0, 64 })
              vim.hl.range(buf, ns, "Comment", { 0, 64 }, { 0, -1 })
              -- Line 1: switch category + execute
              ...
              -- Line 2: copy command + copy to clipboard
          ...
          on_close = function(picker2)
            ---@diagnostic disable: undefined-field
            if picker2.hints_win and vim.api.nvim_win_is_valid(picker2.hints_win) then
              vim.api.nvim_win_close(picker2.hints_win, true)
            end
          end,
        })
      end,
      on_show = function(picker)
        if not picker.hints_win or not vim.api.nvim_win_is_valid(picker.hints_win) then
          ...
          -- Highlight keymaps and descriptions
          local ns = vim.api.nvim_create_namespace("amp_hints")
          -- Line 0: delete category + add new category
          vim.hl.range(buf, ns, "SnacksPickerLabel", { 0, 0 }, { 0, 9 })
          vim.hl.range(buf, ns, "Comment", { 0, 9 }, { 0, 55 })
          vim.hl.range(buf, ns, "SnacksPickerLabel", { 0, 55 }, { 0, 64 })
          vim.hl.range(buf, ns, "Comment", { 0, 64 }, { 0, -1 })
          -- Line 1: edit category name + view prompts
          vim.hl.range(buf, ns, "SnacksPickerLabel", { 1, 0 }, { 1, 9 })
          vim.hl.range(buf, ns, "Comment", { 1, 9 }, { 1, 55 })
          vim.hl.range(buf, ns, "SnacksPickerLabel", { 1, 55 }, { 1, 60 })
          vim.hl.range(buf, ns, "Comment", { 1, 60 }, { 1, -1 })
          -- Line 2: toggle help (single item)
          vim.hl.range(buf, ns, "SnacksPickerLabel", { 2, 0 }, { 2, 4 })
          vim.hl.range(buf, ns, "Comment", { 2, 4 }, { 2, -1 })
        end
      end,
